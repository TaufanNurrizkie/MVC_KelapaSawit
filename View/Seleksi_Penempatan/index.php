        ...
        <main class="flex-1 p-8 bg-gray-50">
            <div class="max-w-7xl mx-auto">
                ...
                <?php if (isset($pengajuan) && is_array($pengajuan) && count($pengajuan) > 0): ?>
                    ...
                    $pengajuanPending = array_filter($pengajuan, function($p) { return ($p['status'] ?? '') === 'pending'; });
                    $totalPending = count($pengajuanPending);
                    $pengajuanAccepted = array_filter($pengajuan, function($p) { return ($p['status'] ?? '') === 'accepted'; });
                    $totalAccepted = count($pengajuanAccepted);
                    $pengajuanRejected = array_filter($pengajuan, function($p) { return ($p['status'] ?? '') === 'rejected'; });
